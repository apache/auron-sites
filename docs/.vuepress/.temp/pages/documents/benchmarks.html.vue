...
<p>Here is the benchmark result of TPC-DS 1TB Dataset, running under Spark-3.5.6 and Auron-6.0.0-preview (dc8d7a9).</p>
<BenchmarkChart /><p>Below is a brief introduction of how we run TPC-DS benchmark with Spark/Auron.</p>
...
<span class="line">mvn package <span class="token parameter variable">-DskipTests</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div></div></div><p>Edit your <code v-pre>$SPARK_HOME/conf/spark-default.conf</code> to enable/disable Auron (see the following conf), then launch benchmark runner.
If benchmarking with Auron, ensure that the Auron jar package is correctly built and moved into <code v-pre>$SPARK_HOME/jars</code>. (<a href="https://github.com/kwai/auron/#build-from-source" target="_blank" rel="noopener noreferrer">How to build Auron?</a>)</p>
