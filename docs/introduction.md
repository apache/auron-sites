...
## Overview
The [Apache Auron](https://github.com/apache/auron) accelerator for Apache Spark leverages native vectorized execution to accelerate query processing. It combines the power of the Apache Arrow-DataFusion library and the scale of the Spark distributed computing framework.
...
### Apache Auron’s Solution
Vectorized execution technology operates by processing data in batches rather than rows, reducing function calls and improving computation performance with SIMD instructions. Apache Auron leverages this technology by integrating the Apache Arrow-DataFusion library with the Spark framework.

Apache Auron checks and translates supported operators in the Spark’s physical plan and generates an equivalent native execution plan, then it passes the generated execution plan to the underlying native engine through JNI calls. The native engine executes the plan with DataFusion framework, which benefits from vectorized execution and has better performance comparing to Spark’s JVM based execution.

### Target User
Apache Auron’s target users are those who want to accelerate Spark SQL/DataFrame queries. Users can install Apache Auron as a Spark client extension. After installing, most SQL queries should run faster without modifying, and save cluster resources.

## Architecture
The architecture design of Apache Auron is as follows.
Apache Auron takes a fully optimized physical plan from Spark, mapping it into equivalent execution plan implemented in native engine, and executes in Spark distributed environment.

![Apache Spark + Apache Auron architecture](./img/auron_architecture.webp)

Apache Auron is composed of the following high-level components:
...
![Apache Auron Native Engine](./img/auron_native_engine.webp)

### Currently Supported Native Operators/Expressions

All supported operators in Apache Auron are listed below. Apache Auron does support fallbacking an operator to Spark execution which has not been implemented, so SQLs containing unsupported operators can still be executed successfully. However, fallbacks takes extra costs, too many fallbacks will slow down the execution.

Most Spark builtin expressions are supported in Apache Auron (by translating to DataFusion-physical-exprs). Apache Auron also supports expression-level fallbacking, which can fallback a single unsupported expression to Spark execution. so SQLs containing some unsupported expressions like UDF/UDTFs can still be optimized.
...
### Source Code
Please see [Apache Auron source code](https://github.com/apache/auron) for more information.

### Cooperators

Apache Auron currently has some users and contributors:
