...
Here is the benchmark result of TPC-DS 1TB Dataset, running under Apache Spark-3.5.6 and Apache Auron-6.0.0-preview (dc8d7a9).

<BenchmarkChart />

Below is a brief introduction of how we run TPC-DS benchmark with Apache Spark/Apache Auron.
...
Edit your `$SPARK_HOME/conf/spark-default.conf` to enable/disable Apache Auron (see the following conf), then launch benchmark runner.
If benchmarking with Apache Auron, ensure that the Apache Auron jar package is correctly built and moved into `$SPARK_HOME/jars`. ([How to build Apache Auron?](https://github.com/apache/auron/#build-from-source))
```bash
