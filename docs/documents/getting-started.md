...
# Getting Started with Apache Auron for Apache Spark

## Build from source

To build Apache Auron from source, follow the steps below:

1. Install Rust

Apache Auron's native execution lib is written in Rust. You need to install Rust (nightly) before compiling.

We recommend using [rustup](https://rustup.rs/) for installation.

2. Install JDK

Apache Auron has been well tested with JDK 8, 11, and 17.
...
You can build Apache Auron either *locally* or *inside Docker* using one of the supported OS images via the unified script: `auron-build.sh`.
...
## Run Apache Spark Job with Apache Auron Accelerator

This section describes how to submit and configure an Apache Spark Job with Apache Auron support.

1. Move the Apache Auron JAR to the Apache Spark client classpath (normally spark-xx.xx.xx/jars/).
...
```
## Integrating Apache Auron with Remote Shuffle Services (RSS)

Apache Auron can be integrated with external Remote Shuffle Services to enhance shuffle performance and improve scalability.
...
Apache Auron can work with Celeborn as a shuffle manager. Integration involves configuring Apache Auron/Apache Spark to use the AuronCelebornShuffleManager and pointing it to the appropriate Celeborn master endpoints and storage locations. This allows Apache Spark jobs running on Apache Auron to leverage Celeborn for distributed shuffling.
...
Similarly, Apache Auron also supports Uniffle, you need to configure Apache Auron/Apache Spark to use the AuronUniffleShuffleManager and specify the Uniffle coordinator endpoints.
